...
await modal.getByLabel("Staff member").selectOption({ label: "Momo Segawa" });
await modal.getByLabel("Day of week").selectOption({ label: "Tuesday" });
await modal.getByLabel("Location").selectOption({ label: "Upper Coomera" });
await modal.getByLabel("Role").selectOption({ label: "Centre Admin" });
...
await page.waitForTimeout(300);
{
  ...
  await modal.getByLabel("Staff member").selectOption({ label: byId[dup.staff_id].name });
  await modal.getByLabel("Day of week").selectOption({ index: dup.day_of_week });
...
await page.getByRole("button", { name: "New shift" }).click();
await page.waitForTimeout(300);
await modal.getByLabel("Staff member").selectOption({ label: "Cass Reid" });
await modal.getByLabel("Day of week").selectOption({ label: "Monday" });
